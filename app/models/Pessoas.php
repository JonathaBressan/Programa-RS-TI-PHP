...
class Pessoas {
    ...
    public function inserirPessoa() {
     $sqlQuery = "INSERT INTO `pessoas` (`cpf`, `nome`, `sobrenome`, `data_nascimento`, `sexo`, `telefone`, `email`, `cep`, `numero`, `bairro`, `cidade`, `rua`, `complemento`, `estado`) VALUES ('?', '?', '?', '?', '?','?', '?', '?', '?', '?', '?', '?', '?', '?');'";
        
        try {
            return Database::query($sqlQuery,[$this->cpf, $this->nome, $this->sobrenome ,$this->data_nascimento, $this->sexo, $this->telefone, $this->email, $this->cep, $this->numero, $this->bairro, $this->cidade, $this->rua, $this->complemento, $this->estado]);
        } catch (\PDOException $e) {
            exit($e->getMessage());
        }
    }
